-- 1. Adiciona o papel "jogador" ao enum (idempotente)
ALTER TYPE public.app_role ADD VALUE IF NOT EXISTS 'jogador';

-- 2. Sincroniza profiles.is_influencer a partir de user_roles
CREATE OR REPLACE FUNCTION public.sync_profile_is_influencer(_user_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  UPDATE public.profiles
     SET is_influencer = EXISTS (
       SELECT 1 FROM public.user_roles
        WHERE user_id = _user_id
          AND role IN ('afiliado','gerente','admin','super_admin')
     )
   WHERE id = _user_id;
END $$;

CREATE OR REPLACE FUNCTION public.trg_sync_profile_is_influencer()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    PERFORM public.sync_profile_is_influencer(OLD.user_id);
    RETURN OLD;
  END IF;
  PERFORM public.sync_profile_is_influencer(NEW.user_id);
  RETURN NEW;
END $$;

DROP TRIGGER IF EXISTS sync_is_influencer ON public.user_roles;
CREATE TRIGGER sync_is_influencer
AFTER INSERT OR UPDATE OR DELETE ON public.user_roles
FOR EACH ROW EXECUTE FUNCTION public.trg_sync_profile_is_influencer();

-- 3. Reescreve handle_new_user: nunca marca is_influencer por link;
--    todo novo cadastro recebe papel "jogador".
...
BEGIN
  ...
  VALUES (
    ...
    ref_owner,
    new_manager,
    false  -- jogador comum por padrão; vira parceiro só ao receber papel afiliado/gerente
  );

  -- Papel padrão: jogador
  INSERT INTO public.user_roles(user_id, role)
    VALUES (NEW.id, 'jogador')
    ON CONFLICT DO NOTHING;

  -- Rede de indicações (mantém comportamento)
  IF ref_owner IS NOT NULL THEN
